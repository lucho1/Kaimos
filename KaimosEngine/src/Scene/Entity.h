...
namespace Kaimos {
	...
	public:
		...
		inline bool operator !=(const Entity& other)	const { return !(*this == other); }											// You are using the above operator, that's elegant :)

		inline operator bool()							const { return m_Entity != entt::null; }
		inline operator entt::entity()					const { return m_Entity; }
		inline operator uint()							const { return (uint)m_Entity; }

		inline uint GetID()								const { return(uint)m_Entity; }
